...
class RustAsset extends Asset {
  ...
  async parse() {
    ...
    let gopkgConfig = await this.getConfig(['Gopkg.toml']);
    let gopkgDir;
    ...
    if (gopkgConfig) {
      ...
      gopkgDir = path.dirname(await config.resolve(this.name, ['Gopkg.toml']));
  ...
  async installGo() {
    ...
    try {
      ...
      throw new Error(
        "Go isn't installed. Visit https://www.golang.org/ for more info"
      );
    }

    // Ensure at least 1.11 toolchain is installed
    let [stdout] = await exec('go', ['version']);
    if (!stdout.includes('')) {
    ...
    if (!stdout.includes(GO_TARGET + ' (installed)')) {
